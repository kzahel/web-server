  ...
  implements ILoggableComponent
{
  ...
  getLogName(): string {
    return (
      this.instanceLogName ??
      (this.constructor as unknown as { logName: string }).logName
    );
  }
...
export function prefixedLogger(
  prefix: string,
  base: Logger = basicLogger(),
): Logger {
  return {
