...
import { fromString, concat } from '../utils/buffer.js'
import type { HttpResponseOptions } from './types.js'

/**
 * Send a complete HTTP response (headers + body) over a socket.
 */
export function sendResponse(socket: ITcpSocket, response: HttpResponseOptions): void {
  ...
  const headerBytes = buildHeaderBytes(response.status, response.statusText, headers)
